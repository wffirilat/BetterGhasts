...
public class MapGenUnderdarkCaves extends MapGenBase {
	...
	protected void func_151543_a(long seed, int chunkX, int chunkZ, Block[] blocks, double p_151543_6_, double p_151543_8_, double p_151543_10_, float p_151543_12_, float p_151543_13_, float p_151543_14_, int p_151543_15_, int p_151543_16_, double p_151543_17_) {
		...
		for (boolean flag = random.nextInt(6) == 0; p_151543_15_ < p_151543_16_; ++p_151543_15_) {
			double d6 = 1.5D + MathHelper.sin(p_151543_15_ * (float) Math.PI / p_151543_16_) * p_151543_12_ * 1.0F;
			double d7 = d6 * p_151543_17_;
			float f5 = MathHelper.cos(p_151543_14_);
			float f6 = MathHelper.sin(p_151543_14_);
			p_151543_6_ += MathHelper.cos(p_151543_13_) * f5;
			p_151543_8_ += f6;
			p_151543_10_ += MathHelper.sin(p_151543_13_) * f5;
			...
			if (flag1 || random.nextInt(4) != 0) {
				...
				double d10 = p_151543_16_ - p_151543_15_;
				double d11 = p_151543_12_ + 2.0F + 16.0F;
				...
				if (p_151543_6_ >= d4 - 16.0D - d6 * 2.0D && p_151543_10_ >= d5 - 16.0D - d6 * 2.0D && p_151543_6_ <= d4 + 16.0D + d6 * 2.0D && p_151543_10_ <= d5 + 16.0D + d6 * 2.0D) {
					...
					if (!flag2) {
						for (k2 = i4; k2 < l1; ++k2) {
							double d13 = (k2 + chunkX * 16 + 0.5D - p_151543_6_) / d6;

							for (j3 = k4; j3 < j2; ++j3) {
								double d14 = (j3 + chunkZ * 16 + 0.5D - p_151543_10_) / d6;
								int k3 = (k2 * 16 + j3) * 128 + i2;

								for (int l3 = i2 - 1; l3 >= j4; --l3) {
									double d12 = (l3 + 0.5D - p_151543_8_) / d7;
